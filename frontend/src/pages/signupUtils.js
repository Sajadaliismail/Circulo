...
export const validationSchema = object({
    ...
    password: string().min(8, 'Password must be at least 8 characters long').matches(/^(?=.*[a-zA-Z])(?=.*[0-9])/, 'Password must be alphanumeric').required('Password is required'),
    cnf_password: string().oneOf([ref('password'), null], 'Passwords must match')
        .required('Confirm Password is required'),
    birthMonth: string().required('Please enter birth month'),
    birthYear: number().required('Please enter birth year').test('age', 'You must be at least 12 ', function (value) {
...
export const years = Array.from(new Array(currentYear - 1950 + 1), (val, index) => 1950 + index);
